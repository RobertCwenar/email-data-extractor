...
async def test_main():
    ...
    with (
        patch("main.AIService", return_value=mock_ai_instance),
        patch("main.Database", return_value=mock_db_instance),
        patch("main.FilterService", return_value=mock_filter_instance),
        patch("main.EmailParser", return_value=mock_parser_instance),
    ):
        await main()
